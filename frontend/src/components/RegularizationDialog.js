        ...
        {preview && !loading && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-2">
              ...
              <Card className="border-[#0055FF]"><CardContent className="p-3"><div className="text-[10px] uppercase text-blue-700">Repartition</div><div className="text-sm mt-1 font-semibold">{preview.summary.owners_debiteurs} D / {preview.summary.owners_crediteurs} C</div></CardContent></Card>
            </div>
                  ...
                  <TableBody>
                    {preview.per_owner?.map((p, i) => (
                      <TableRow key={i} data-testid={`regul-row-${i}`}>
                        ...
                        <TableCell className="font-mono text-[#0055FF] text-xs">{p.vcs_code}</TableCell>
            ...
            <div className="mt-4 flex items-center justify-between border-t pt-3">
              {step === 1 ? (
                ...
                <>
                  ...
                  <Button onClick={() => { onDone?.(); onClose(); }} className="bg-[#0055FF] hover:bg-[#0040CC]">Terminer</Button>
                </>
              )}
            </div>
          </>
