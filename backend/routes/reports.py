...
async def compute_bilan_data(db, copropriete_id: str, date_to: Optional[str] = None,
    ...
    for entry in entries:
        # iter90gz : les OD de financement par fonds de reserve creditent
        # un compte de classe 6 (reduisent les charges) et debitent un
        # compte de reserve (classe 1). Ces OD DOIVENT etre integrees
        # dans `total_charges` : sans cela, la reserve (Passif) baisse
        # sans compensation par le boni du 499 (Passif), ce qui
        # DESEQUILIBRE le bilan (ecart = montant du financement).
        # Regle : Charges nettes = Debit cl.6 - Credit cl.6 (TOUS journaux
        # confondus, AC + OD + VE). Le boni 499 = Produits - Charges nettes
        # augmente donc du meme montant que la reserve diminue -> equilibre
        # Actif = Passif preserve.
        for line in entry.get("lines", []):
            acc = line.get("account_number", "")
            if not acc:
                continue
